...
function NewComp() {
  ...
  async function handleSubmit(e) {
    ...
    setLoading(true);
    setData([]);
    let myQuery = query.trim().split(" ");
    console.log("📝 Query words:", myQuery);
    try {
      const response = await axios.get('https://algosearch-backend.onrender.com/search', {
        headers: {
      ...
      <main className="w-screen h-screen">
        <div className="flex justify-between items-center w-[95vw] mx-auto mt-4" >
          <div className="flex justify-start items-center gap-2 ">
            ...
            <h1 className="monu text-sm md:text-2xl tracking-wider caret-transparent"><span className='text-purple-600'>Leet</span>Search</h1>
            <p className="text-purple-600 border-[1px] border-purple-600 rounded-md p-1 text-xs font-extrabold comfort">Beta</p>
          </div>
          ...
          <div className="w-full h-[80vh] rounded-xl bg-[#eaeaeaec] dark:bg-[#232327] mt-5 overflow-y-scroll p-2">
            ...
            <div class="absolute bottom-0 left-0 w-full border-t md:border-t-0 dark:border-white/20 md:border-transparent md:dark:border-transparent md:bg-vert-light-gradient bg-white dark:bg-gray-800 md:!bg-transparent dark:md:bg-vert-dark-gradient pt-2 md:-left-2 "><form onSubmit={handleSubmit} class="stretch mx-2 flex flex-row gap-3 last:mb-2 md:mx-4 md:last:mb-6 lg:mx-auto lg:max-w-2xl xl:max-w-3xl"><div class="relative flex h-full flex-1 items-stretch md:flex-col" role="presentation"><div class="flex flex-col w-full py-[10px] flex-grow md:py-4 md:pl-4 relative border border-black bg-white dark:border-gray-50 dark:text-white dark:bg-[#131314] rounded-[30px] shadow-xs dark:shadow-xs"><textarea value={query} onChange={(e) => setQuery(e.target.value)} id="prompt-textarea" tabindex="0" data-id="root" rows="1" placeholder="Search here ..." class="m-0 w-full resize-none border-0 bg-transparent p-0 pr-10 focus:ring-0 focus-visible:ring-0 dark:bg-transparent md:pr-12 pl-3 md:pl-0 outline-none placeholder-gray-900 dark:placeholder-gray-50 text-sm"></textarea>
              <button onClick={(e) => {
                e.preventDefault();
                handleSubmit(e);
              }} disabled={!query} class="absolute p-1 rounded-md md:bottom-3 md:p-2 md:right-3 dark:hover:bg-gray-900 dark:disabled:hover:bg-transparent right-2 disabled:text-gray-400 disabled:opacity-50 enabled:bg-purple-600 enabled:text-white text-white bottom-1.5 transition-colors"><svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" class="w-6 h-6 dark:stroke-white stroke-black ">
                <path stroke-linecap="round" stroke-linejoin="round" d="M6 12L3.269 3.126A59.768 59.768 0 0121.485 12 59.77 59.77 0 013.27 20.876L5.999 12zm0 0h7.5" />
              </svg>
                {/* Text */}
              </button></div></div></form><div class="px-3 pb-3 pt-2 text-center text-xs text-gray-600 dark:text-gray-300 md:px-4 md:pb-6 md:pt-3"><span>LeetSearch in beta testing may produce inaccurate predictions. <a href="https://www.linkedin.com/in/jayee-das-64653822a/" target="_blank" rel="noreferrer" class="underline">Developer LeetSearch</a></span></div></div>
          </div>
        </div>
      </main>
    </>
